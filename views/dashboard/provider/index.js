          ...
          if (response.errors.length > 0) {
              ...
              upload({
                  ...
                  .then(function (response) {
                    // response.data.errors = [{errors: true}];
                    if(response.data.errors.length > 0) {
                      var modalData = {
                        modalTitle: 'Error',
                        modalBody: 'Something went wrong when trying to create a new patient.'
                      }
                ...
                upload({
                    ...
                    .then(function (response) {
                      ...
                      if(response.data.errors.length > 0) {
                        var modalData = {
                          modalTitle: 'Error',
                          modalBody: 'Something went wrong. Patient was not deleted.'
                        }
                        commonFnSrvc.globalModal(modalData);
                      }
                    })
    ...
    ModalService.showModal({
        ...
        controller: function ($element) {
            ...
            this.refundAndArchive = function () {
              apiSrvc.getData($rootScope.apiUrl + '/Default.aspx?remotemethodaddon=appDemo2SendRequestRefundandArchiveEmail&PatientID=' + $scope.data.id).then(function(response) {
                if(response.errors.length > 0) {
                  $scope.showErrorMsg = true;
                  $scope.errorMsg = "Something went wrong.  Please contact the appDemo2 Administrator for assistance."
                }
                else {
                   $element.modal('hide');
                   $scope.getDashboardData();
