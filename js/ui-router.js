app.config(function($stateProvider, $urlRouterProvider, $locationProvider) {
  ...
  $stateProvider
    ...
    .state('createRx', {
        ...
        url: '/createRx/{id:int}/{selectedTile:int}/:prescriptionGuid/:RxTreatmentStage/:hasAppliance/:freeGuide',
...
app.run(function ($rootScope, $window, apiSrvc, $interval) {
  $rootScope.logOut = function () {
    apiSrvc.getData($rootScope.apiUrl + '/Default.aspx?remotemethodaddon=appDemo2ProcessLogout').then(function (response) {
        window.location = $rootScope.cloudUrl;
    });
  };
  $interval(callAtInterval, 600*1000, [18]); //call every 10 minutes, call 18 times (18 x 10 min = 3 hours + s3Bucket timeout (1hr)= 4 hours
  ...
  $rootScope.myAccountLink = function () {
      apiSrvc.getData($rootScope.apiUrl + '/Default.aspx?remotemethodaddon=appDemo2GetUserInformation').then(function (response) {
          if (response.errors.length > 0) {
              ...
              window.location = $rootScope.cloudUrl + '/My-Account';
          }
      })
  };
});
